...
function security_check() {
  ...
  for (var guest of queue) {
    if(guest.alcohol === 0 && guest.guns === 0) {
      entries.push(guest.name);
    } else if(guest.guns > 0) {
      security_alchol_loot += guest.alcohol;
      guest.alcohol = 0;
      guest.guns = 0;
      watchlist.push(guest.name);
    } else if(guest.alcohol > 0) {
      security_alchol_loot += guest.alcohol;
      guest.alcohol = 0;
      entries.push(guest.name);
    }
